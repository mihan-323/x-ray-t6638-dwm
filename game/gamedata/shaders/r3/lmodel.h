...
#define LMODEL_H
	...
	float4 plight_local(float mtl, float3 position, float3 normal, float3 lposition, float lightrangersq)
	{
		float3 light2point = position - lposition; // light to point

		float3 vec2eye = -normalize(position); // vector to eye

		float3 vec2light = -normalize(light2point); // vector to light
