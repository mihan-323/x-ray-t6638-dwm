...
void R_dsgraph_structure::r_dsgraph_render_hud	()
{
	...
	/*CTAA TAA;
	taa_matrices M = TAA.get_xforms();
	static Fmatrix m_tP = { };
	RCache.xforms_taa.m_p = m_tP;
	RCache.xforms_taa.m_vp.mul(m_tP, M.m_V);
	RCache.xforms_taa.m_wvp.mul(m_tP, M.m_WV);
	m_tP = Device.mProject;*/
...
void R_dsgraph_structure::r_dsgraph_render_hud_ui()
{
	...
	{
		RImplementation.Target->u_setrt(RImplementation.Target->rt_Color_ms);
		RImplementation.Target->u_setzb(RImplementation.Target->rt_MSAA_depth);
	}
	else
	{
