...
void	CRenderTarget::phase_luminance()
{
	...
	u_setrt(rt_LUM_pool[gpu_id * 2 + 1]);
	u_setzb(FALSE);
	//RImplementation.rmNormal();
	{
		// Build filter-kernel
		...
		f_luminance_adapt = .9f * f_luminance_adapt + .1f * Device.fTimeDelta * r__tonemap_adaptation;
		float		amount = opt(R__USE_TONEMAP) ? r__tonemap_amount : 0;
		Fvector3	_none, _full, _result;
		_none.set(1, 0, 1);
		_full.set(r__tonemap_middlegray, 1.f, r__tonemap_low_lum);
