...
{
	float		a		= 1-(W->ttl/ r__wallmark_ttl);
...
ICF void FlushStream(ref_geom hGeom, ref_shader shader, u32& w_offset, FVF::LIT*& w_verts, FVF::LIT*& w_start, BOOL bSuppressCull)
{
	...
	if (w_count)			
	{
		...
		if (bSuppressCull)		RCache.set_CullMode (D3D_CULL_NONE);
		RCache.Render			(D3DPT_TRIANGLELIST,w_offset,w_count/3);
		if (bSuppressCull)		RCache.set_CullMode	(D3D_CULL_BACK);
...
//	if (marks.empty())			return;
	...
	Device.mProject._43			-= r__wallmark_shift_pp;
	RCache.set_xform_world		(Fidentity);
	RCache.set_xform_project	(Device.mProject);

	Fmatrix	mSavedView			= Device.mView;
	Fvector	mViewPos			;
			mViewPos.mad		(Device.vCameraPosition, Device.vCameraDirection, r__wallmark_shift_v);
