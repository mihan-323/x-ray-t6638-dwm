...
void dx10StateManager::Reset()
{
	...
	StateUtils::ResetDescription(m_RDesc);
	StateUtils::ResetDescription(m_DSDesc);
	StateUtils::ResetDescription(m_BDesc);
...
void dx10StateManager::SetCullMode(u32 Mode)
{
	...
	if (m_RDesc.CullMode!= (D3D_CULL_MODE)Mode)
	{
		m_bRSChanged = true;
	...
	else
	{
		if (m_bRSChanged)
		{
			...
			if (m_pRState)
				...
				StateUtils::ResetDescription(tmpDesc);
