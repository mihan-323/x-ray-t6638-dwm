...
{
	void 				create			(D3DVERTEXELEMENT9* decl, ID3DBuffer* vb, ID3DBuffer* ib);
	void				create			(u32 FVF				, ID3DBuffer* vb, ID3DBuffer* ib);
...
public:
	ref_selement		E		[R_BLRENDER_STAGES];	// R2 - 0=deffer,			1=norm_lod1(normal),	2=psm,			3=ssm,		4=dsm
						~Shader			();
	BOOL				equal			(Shader& S);
	BOOL				equal			(Shader* S);
};
