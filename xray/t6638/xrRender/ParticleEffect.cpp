...
void CParticleEffect::Render(float )
{
	...
	if(p_cnt>0){
		if (m_Def&&m_Def->m_Flags.is(CPEDef::dfSprite)){
			...
			if ( p_cnt < nWorkers * 64 ) // SULTAN: поставь 64
				nWorkers = 1;
			...
			if (dwCount)    
			{
				...
				if(GetHudMode())
				{
					RDEVICE.mProject.build_projection(	deg2rad(psHUD_FOV* float(Device.fFOV)),
														Device.fASPECT, 
														VIEWPORT_NEAR, 
                ...
                RCache.set_CullMode		(m_Def->m_Flags.is(CPEDef::dfCulling)?(m_Def->m_Flags.is(CPEDef::dfCullCCW)? D3D_CULL_BACK : D3D_CULL_FRONT): D3D_CULL_NONE);
