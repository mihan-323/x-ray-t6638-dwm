...
void SimulatorStates::UpdateDesc( D3D_RASTERIZER_DESC &desc ) const
{
	for (u32 it=0; it<States.size(); it++)
	{
		...
		if (S.type==0)
		{
			...
			case D3DRS_CULLMODE:
				desc.CullMode = (D3D_CULL_MODE)S.v2;
				break;
...
void SimulatorStates::UpdateDesc( D3D_DEPTH_STENCIL_DESC &desc ) const
{
	for (u32 it=0; it<States.size(); it++)
	{
		...
		if (S.type==0)
		{
			...
			case D3DRS_ZFUNC:
				desc.DepthFunc = (D3D_COMPARISON_FUNC)S.v2;
				break;
			...
			case D3DRS_STENCILFAIL:
				desc.FrontFace.StencilFailOp = (D3D_STENCIL_OP)S.v2;
				break;

			case D3DRS_STENCILZFAIL:
				desc.FrontFace.StencilDepthFailOp = (D3D_STENCIL_OP)S.v2;
				break;

			case D3DRS_STENCILPASS:
				desc.FrontFace.StencilPassOp = (D3D_STENCIL_OP)S.v2;
				break;

			case D3DRS_STENCILFUNC:
				desc.FrontFace.StencilFunc = (D3D_COMPARISON_FUNC)S.v2;
				break;

			case D3DRS_CCW_STENCILFAIL:
				desc.BackFace.StencilFailOp = (D3D_STENCIL_OP)S.v2;
				break;

			case D3DRS_CCW_STENCILZFAIL:
				desc.BackFace.StencilDepthFailOp = (D3D_STENCIL_OP)S.v2;
				break;

			case D3DRS_CCW_STENCILPASS:
				desc.BackFace.StencilPassOp = (D3D_STENCIL_OP)S.v2;
				break;

			case D3DRS_CCW_STENCILFUNC:
				desc.BackFace.StencilFunc = (D3D_COMPARISON_FUNC)S.v2;
				break;
			}
		}
	}
...
void SimulatorStates::UpdateDesc( D3D_BLEND_DESC &desc ) const
{
	for (u32 it=0; it<States.size(); it++)
	{
		...
		if (S.type==0)
		{
			...
			case D3DRS_SRCBLEND:
				for ( int i=0; i<8; ++i)
					desc.RenderTarget[i].SrcBlend = (D3D_BLEND)S.v2;
				break;

			case D3DRS_DESTBLEND:
				for ( int i=0; i<8; ++i)
					desc.RenderTarget[i].DestBlend = (D3D_BLEND)S.v2;
				break;
			...
			case D3DRS_BLENDOP:
				for ( int i=0; i<8; ++i)
					desc.RenderTarget[i].BlendOp = (D3D_BLEND_OP)S.v2;
				break;

			case D3DRS_SRCBLENDALPHA:
				for ( int i=0; i<8; ++i)
					desc.RenderTarget[i].SrcBlendAlpha = (D3D_BLEND)S.v2;
				break;

			case D3DRS_DESTBLENDALPHA:
				for ( int i=0; i<8; ++i)
					desc.RenderTarget[i].DestBlendAlpha = (D3D_BLEND)S.v2;
				break;

			case D3DRS_BLENDOPALPHA:
				for ( int i=0; i<8; ++i)
					desc.RenderTarget[i].BlendOpAlpha = (D3D_BLEND_OP)S.v2;
				break;
...
//					desc.Filter |= MagfilterLinear;
			...
			case D3DSAMP_ADDRESSU:	/* D3D_TEXTURE_ADDRESS_MODE for U coordinate */
				desc.AddressU = (D3D_TEXTURE_ADDRESS_MODE)S.v3;
				break;

			case D3DSAMP_ADDRESSV:	/* D3D_TEXTURE_ADDRESS_MODE for V coordinate */
				desc.AddressV = (D3D_TEXTURE_ADDRESS_MODE)S.v3;
				break;

			case D3DSAMP_ADDRESSW:	/* D3D_TEXTURE_ADDRESS_MODE for W coordinate */
				desc.AddressW = (D3D_TEXTURE_ADDRESS_MODE)S.v3;
				break;
