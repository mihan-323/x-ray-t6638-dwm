...
{
	tsky0	= DEV->_CreateTexture(tex_t_sky_0);
	tsky1	= DEV->_CreateTexture(tex_t_sky_1);
}
...
{
	tonemap	= DEV->_CreateTexture(tex_t_LUM_dest);	//. hack
}
...
void dxEnvironmentRender::RenderSky(CEnvironment &env)
{
	...
	{
		s_sky.create(&b_sky);
		g_skybox.create(v_skybox_fvf, RCache.Vertex.Buffer(), RCache.Index.Buffer());
		g_clouds.create(v_clouds_fvf, RCache.Vertex.Buffer(), RCache.Index.Buffer());
	...
	// Fill vertex buffer
	v_skybox* pv				= (v_skybox*)	RCache.Vertex.Lock	(12,g_skybox.stride(),v_offset);
	for (u32 v=0; v<12; v++)	pv[v].set		(hbox_verts[v*2],C,hbox_verts[v*2+1]);
	RCache.Vertex.Unlock		(12,g_skybox.stride());

	ref_selement skybox = RImplementation.phase == RImplementation.PHASE_PLANAR ? s_sky->E[SE_SKYBOX1] : s_sky->E[SE_SKYBOX];

	// Render
	RCache.set_xform_world		(mSky);
	RCache.set_Geometry			(g_skybox);
	RCache.set_Element			(skybox);
...
void dxEnvironmentRender::RenderClouds(CEnvironment &env)
{
	...
	// Fill vertex buffer
	v_clouds* pv				= (v_clouds*)	RCache.Vertex.Lock	(env.CloudsVerts.size(),g_clouds.stride(),v_offset);
	for (FvectorIt it=env.CloudsVerts.begin(); it!=env.CloudsVerts.end(); it++,pv++)
		pv->set					(*it,C0,C1);
	RCache.Vertex.Unlock		(env.CloudsVerts.size(), g_clouds.stride());

	ref_selement clouds = RImplementation.phase == RImplementation.PHASE_PLANAR ? s_sky->E[SE_CLOUDS1] : s_sky->E[SE_CLOUDS];

	// Render
	RCache.set_xform_world		(mXFORM);
	RCache.set_Geometry			(g_clouds);
	RCache.set_Element			(clouds);
	dxEnvDescriptorMixerRender	&mixRen = *(dxEnvDescriptorMixerRender*)&*env.CurrentEnv->m_pDescriptorMixer;
