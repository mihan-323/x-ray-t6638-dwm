...
void					CTexture::surface_set	(ID3DBaseTexture* surf )
{
	...
	if (pSurface)
	{
		...
		if (D3D_RESOURCE_DIMENSION_TEXTURE2D == type )
		{
			...
			switch (desc.Format)
			{
			case DXGI_FORMAT_R32G32_TYPELESS:	ViewDesc.Format = DXGI_FORMAT_R32_FLOAT_X8X24_TYPELESS;		break;
...
void CTexture::apply_theora(u32 dwStage)
{
	if (pTheora->Update(m_play_time!=0xFFFFFFFF?m_play_time:Device.dwTimeContinual))
	{
		...
		D3D_MAPPED_SUBRESOURCE	mapData;
		RECT rect;
		rect.left			= 0;
		rect.top			= 0;
...
void CTexture::Load		()
{
	...
	// Check for OGM
	string_path			fn;
	if ((FS.exist(fn,"$game_textures$",*cName,".ogv")) || (FS.exist(fn,"$game_textures$",*cName,".ogm"))){
		// AVI
		...
		if (!pTheora->Load(fn)) {
			xr_delete(pTheora);
			FATAL				("Can't open video stream .ogm\\.ogv");
		} else {
		...
		if (FS.exist(fn,"$game_textures$",*cName,".avi")){
			...
			if (!pAVI->Load(fn)) {
				xr_delete(pAVI);
				FATAL				("Can't open video stream .avi");
			} else {
