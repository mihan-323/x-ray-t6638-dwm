...
//.			position.mad(direction,0.025f*radius);
	...
	if (MODE & IRender_ObjectSpecific::TRACE_LIGHTS )	{
		...
		float	hemi_light = (lacc.x + lacc.y + lacc.z)/3.0f * r__dhemi_light_scale;
		...
		for (size_t i = 0; i < NUM_FACES; ++i)
		{
			hemi_cube[i] += hemi_cube_light[i]*(1- r__dhemi_light_flow) + r__dhemi_light_flow *hemi_cube_light[(i+NUM_FACES/2)%NUM_FACES];
...
void	CROS_impl::update_smooth	(IRenderable* O)
{
	...
	float	l_f				=	Device.fTimeDelta* r__dhemi_smooth;
	clamp	(l_f,0.f,1.f)	;
	float	l_i				=	1.f-l_f							;
...
void CROS_impl::calc_sky_hemi_value(Fvector& position, CObject* _object)
{
	...
	if	(MODE & IRender_ObjectSpecific::TRACE_HEMI)	
	{
		...
		sky_rays_uptodate	+= r__dhemi_count;
	...
	hemi_value				*= r__dhemi_sky_scale;

	for (int it=0; it<result_count; it++)
	{
		if (result[it])
		{
			accum_hemi(hemi_cube, Fvector3().set(hdir[it][0], hdir[it][1], hdir[it][2]), r__dhemi_sky_scale);
		}
	}
}
