...
void CRenderTarget::phase_bloom	()
{
	...
	// Targets
	u_setrt(rt_Bloom_1);
	u_setzb(FALSE);		// No need for ZBuffer at all
	...
	// Transfer into Bloom1
	{
		...
		float s = r__bloom_threshold;											// scale
		f_bloom_factor = .9f * f_bloom_factor + .1f * r__bloom_speed * Device.fTimeDelta;	// speed
	...
	// Transfer into Bloom2, horizontal filter
	{
		...
		// Perform filtering
		Fvector4	w0, w1;
		float		kernel = r__bloom_kernel_g;
		CalcGauss_wave(w0, w1, kernel, kernel / 3.f, r__bloom_kernel_scale);
		u_setrt(rt_Bloom_2);
		u_setzb(FALSE);		// No need for ZBuffer at all
	...
	// Transfer into Bloom1, vertical filter
	{
		...
		// Perform filtering
		Fvector4	w0, w1;
		float		kernel = r__bloom_kernel_g * float(Device.dwHeight) / float(Device.dwWidth);
		CalcGauss_wave(w0, w1, kernel, kernel / 3.f, r__bloom_kernel_scale);
		u_setrt(rt_Bloom_1);
		u_setzb(FALSE);				// No need for ZBuffer at all
