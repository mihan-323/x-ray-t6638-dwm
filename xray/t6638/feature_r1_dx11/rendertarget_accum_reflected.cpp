...
void CRenderTarget::accum_reflected		(light* L)
{
	...
	if (bIntersect)	RCache.set_CullMode		(D3D_CULL_FRONT);		// back
	else			RCache.set_CullMode		(D3D_CULL_BACK);		// front
...

	{
		...
		{
			// per pixel 
			RCache.set_Stencil(TRUE, D3D_COMPARISON_EQUAL, 0x01, 0x81, 0x00);
			draw_volume(L);

			// per sample
			RCache.set_Stencil(TRUE, D3D_COMPARISON_EQUAL, 0x81, 0x81, 0x00);
			if (bIntersect)	RCache.set_CullMode(D3D_CULL_FRONT);		// back
			else			RCache.set_CullMode(D3D_CULL_BACK);		// front
			draw_volume(L);
