...
void dx103DFluidManager::Reset()
{
	...
	for(int rtIndex=0; rtIndex<NUM_OWN_RENDER_TARGETS; rtIndex++)
	{
		RCache.clear_RenderTargetView(pRenderTargetViews[rtIndex], color );
	}
}
...
void dx103DFluidManager::Update( dx103DFluidData &FluidData, float timestep )
{
	...
	RCache.set_Viewport(&rtViewport);
...
void dx103DFluidManager::AdvectColorBFECC( float timestep, bool bTeperature )
{
	...
	RCache.clear_RenderTargetView(pRenderTargetViews[RENDER_TARGET_TEMPVECTOR], color );
	RCache.clear_RenderTargetView(pRenderTargetViews[RENDER_TARGET_TEMPSCALAR], color );
...
void dx103DFluidManager::UpdateObstacles( const dx103DFluidData &FluidData, float timestep )
{
	...
	//	Reset data
	float color[4] = {0, 0, 0, 0 };
	RCache.clear_RenderTargetView(pRenderTargetViews[RENDER_TARGET_OBSTACLES], color );
	RCache.clear_RenderTargetView(pRenderTargetViews[RENDER_TARGET_OBSTVELOCITY], color );
